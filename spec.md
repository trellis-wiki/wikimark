...
**Multi-line template calls.** A template call MAY span multiple source
lines. Newlines and surrounding indentation inside `{{...}}` are
treated as inter-argument whitespace — the call is equivalent to the
same name and arguments written on a single line. This is useful for
long argument lists (e.g. infoboxes):
...
The opening `{{` and closing `}}` are balanced with nesting support, so
a multi-line outer call that contains nested inline templates is
parsed as a single call whose interior is rewritten accordingly.
...
WikiMark is **case-preserving at the parser level**: the case of the
original text MUST be carried through to the URL without
transformation. A processor MUST NOT perform first-letter
capitalization (that is a MediaWiki-specific engine convention, not
a markup rule).
...
**Case-sensitive matching is out of scope for this spec.** Whether a
wiki engine treats `[[Home]]` and `[[home]]` as the same page, or as
different pages, is engine policy — not a parser concern. Engines
that fold case MUST document the canonical form they apply (for
example, "first-letter-uppercase" or "lowercase"). The spec does not
require, permit, or forbid any particular choice; a conformant
processor simply emits the URL as the parser computed it and leaves
lookup semantics to the engine.

This section previously mandated case-sensitive behavior, which
overstepped into engine territory. The mandate is withdrawn; the
underlying parser behavior (preserve case as written) is unchanged.

### 13.4 Unicode normalization

Page titles SHOULD be normalized to Unicode NFC (Canonical
Decomposition, followed by Canonical Composition) before URL
generation. Implementations that skip normalization are conformant
but MUST document the omission.

Rationale: real-world wiki titles are overwhelmingly ASCII or
already-normalized Unicode. Mandating NFC would force every
implementation to bundle a Unicode normalization table or link
libunicode, which is disproportionate for an edge case (combining
marks in user-typed page titles). Engines that want strict NFC can
normalize on the engine side without changing the parser.

Implementations that do not normalize MUST handle titles
containing non-NFC Unicode byte-for-byte. Two visually identical
titles differing only in NFC vs. NFD representation will be treated
as distinct pages under such an implementation — engines are
responsible for disambiguating if this matters.
...
WikiMark processors accept untrusted input. Because the syntax
includes recursive template expansion and user-authored frontmatter,
a naïve implementation is trivially vulnerable to denial-of-service
via pathological input: deeply nested templates, exponential
transclusion, YAML alias bombs, or gigabyte-sized frontmatter
values.

This section defines the **normative resource limits** every
conformant processor MUST enforce, and the **threat model** the
spec assumes engines operate under.

### 15.1 Threat model

The markup language assumes:

- **Input is untrusted.** Any character sequence may appear in any
  position of a WikiMark source document.
- **The processor runs in a shared process.** Exhausting its memory,
  stack, or wall-clock time is a real attack.
- **Output may be served to unrelated users.** Content embedded in
  the rendered output reaches other readers.
- **Engines provide resolvers** (`resolve_variable`,
  `resolve_template`, `resolve_embed`) that the processor calls into
  synchronously. Engine resolvers are responsible for their own
  authorization, sandboxing, and rate limiting — the processor
  treats their return values as authoritative for the purpose of
  inclusion in output.

### 15.2 Mandatory resource limits

A conformant processor MUST enforce each of the following limits.
An implementation MAY expose its numeric values as configuration,
but MUST NOT allow any limit to be disabled entirely. The
**recommended** values in the right column are suitable for general-
purpose wiki content; a processor serving a restricted niche (e.g.,
a wiki with only short article pages) MAY choose tighter values.

| Limit | Scope | Recommended value |
|---|---|---|
| Maximum input document size | One render call | 10 MB |
| Maximum frontmatter block size | One render call | 2 MB |
| Maximum YAML nesting depth | One frontmatter block | 64 |
| Maximum YAML alias references | One frontmatter block | 100 |
| Maximum YAML scalar length | One frontmatter value | 1 MB |
| Maximum template recursion depth | One render call | 40 |
| Maximum template expansion count | One render call | 500 |
| Maximum single expansion output | One expansion | 2 MB |
| Maximum rendered output size | One render call | 10 MB |
| Render wall-clock timeout | One render call | 30 seconds |

When any limit is exceeded, the processor MUST halt expansion of
the offending construct (or the whole render, if the limit is
render-scoped) and emit a `<span class="wm-error">` indicator at
the point where the limit was hit. The rest of the document MUST
continue rendering where it is possible to do so.

### 15.3 Template expansion

Template expansion is the primary attack surface. A malicious or
buggy template can recurse into itself, expand exponentially via
argument multiplication, or produce gigabytes of output from a
trivial input.

**Recursion detection.** Processors MUST detect direct template
self-reference (A → A) and indirect cycles (A → B → A). On
detection, expansion MUST halt at the cycle boundary and an error
indicator MUST be emitted. Cycle detection is scoped per render
call; it does NOT require tracking state across renders.

**Expansion count.** The total number of template expansions
evaluated during a single render MUST be bounded (recommended: 500,
§15.2). This prevents expansion bombs where each expansion emits
multiple further expansions.

**Output size per expansion.** Each expansion's output MUST be
bounded (recommended: 2 MB). This prevents a single pathological
template from producing gigabytes of output even when the
expansion-count limit has room.

### 15.4 Path traversal

Engine resolvers MAY load content from the filesystem (templates,
embedded pages). Implementations MUST prevent traversal outside the
configured root:

- Template names and embed targets MUST be rejected if they contain
  `..` path segments, absolute paths, or path separators that would
  resolve outside the configured directory.
- Symbolic links that resolve outside the root MUST be treated as
  missing.
- Canonicalization MUST occur before the access check so that
  percent-encoded or Unicode-alternate forms cannot bypass it.

### 15.5 Module safety

When a processor supports module-backed templates (see §11.4), it
MUST:

1. Document which module languages and execution environments are
   supported, and how each is sandboxed.
2. Restrict module resolution to a configured list of trusted
   module sources (directories, URLs, or registered names).
3. Enforce wall-clock and memory limits on each module invocation
   equivalent to or stricter than the limits in §15.2.
4. Sanitize module output according to §B.7 before inclusion in
   the rendered document.

A processor that does NOT support module-backed templates SHOULD
state that in its documentation so engines can reason about which
features are available.

### 15.6 YAML frontmatter

YAML is a large attack surface. Processors MUST enforce:

- **Nesting depth** (recommended: 64). Nested maps and sequences
  deeper than this MUST be rejected.
- **Alias count** (recommended: 100). A single document MUST NOT
  be allowed to expand via aliases into arbitrarily many nodes.
  This is the canonical YAML "billion laughs" defense.
- **Scalar length** (recommended: 1 MB). No single YAML scalar
  may exceed this.
- **Document size** (recommended: 2 MB). The frontmatter block
  itself is bounded in bytes before YAML parsing begins.

A YAML document that violates these limits MUST cause the entire
frontmatter block to be rejected. Body content is processed as if
no frontmatter were present; the processor MAY emit a diagnostic
via out-of-band channels (logs, an engine callback).

Processors MUST NOT honor YAML features that reference external
resources (tags pointing at URIs, custom type constructors that
load Python/Ruby/etc. modules). Frontmatter is data, not code.

### 15.7 Variable expansion

The `${...}` variable system is frontmatter-scoped (see §8). The
processor MUST NOT extend variable resolution to:

- Environment variables (e.g., `${HOME}`)
- Filesystem paths (e.g., `${/etc/passwd}`)
- Process state (PID, username, wall-clock time)
- Network resources

Variable values are text. When the resolved value is inserted into
an HTML attribute context (e.g., a wiki link's `href`, a template
call's argument value), the processor MUST HTML-escape the value
before insertion.

### 15.8 Output sanitization

The rendered output MAY contain HTML if the processor is invoked
with a permissive option (e.g., cmark-gfm's `CMARK_OPT_UNSAFE`). In
this mode, wiki content can include `<script>` and other dangerous
elements.

Processors that expose a permissive mode to authors who write
untrusted content MUST combine it with an output-sanitization
pass (e.g., a bleach-style allowlist). Processors that expose
permissive mode only to trusted authors MAY omit sanitization but
MUST document that decision.

See Appendix B.7 for the default-safe output rules.

### 15.9 Denial of service

Processors MUST enforce a wall-clock timeout on each render call
(recommended: 30 seconds). A render that exceeds the timeout MUST
be halted and an error indicator emitted for the incomplete
portion.

Processors SHOULD enforce a concurrent-render limit per caller
(per-user or per-session) to prevent a single client from starving
the service.

### 15.10 Error indicator

The error indicator format (§14.3) is used across §15:

```html
<span class="wm-error">description</span>
```

The `description` SHOULD briefly name the exceeded limit
(`template-recursion-depth`, `yaml-nesting-depth`, `render-timeout`,
etc.) to aid debugging without revealing sensitive implementation
detail.
